...
#[inline]
fn butterfly5(x: [Complex<f32>; 5]) -> [Complex<f32>; 5] {
...
fn mixed100(x: [Complex<f32>; 100]) -> [Complex<f32>; 100] {
    ...
    for factor in [5, 4] {
        ...
        for i in 0..cross_fft_columns {
            for k in 1..factor {
                let twiddle = compute_twiddle(i * k, cross_fft_len, FftDirection::Forward);
...
fn compute_twiddle(index: usize, fft_len: usize, direction: FftDirection) -> Complex<f32> {
    let constant = -2.0 * PI / fft_len as f32;
    let angle = constant * index as f32;

    let result = Complex::new(angle.cos(), angle.sin());
